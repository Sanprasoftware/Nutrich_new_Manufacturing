frappe.ui.form.on('Sales Invoice Item', {
	custom_freight(frm,cdt,cdn) {
        ...
        data.custom_fob_value = data.net_amount - data.custom_freight;

        data.custom_fob_value_inr = data.custom_fob_value * frm.doc.conversion_rate;

        data.custom_duty_drawback_amount = data.custom_fob_value_inr * data.custom_duty_drawback_rate / 100;
        
        data.custom_rodtep_value = data.custom_fob_value_inr * data.custom_rodtep_rate / 100;

        frm.refresh_field('items');
	},
    custom_insurance(frm,cdt,cdn) {
        let data = locals[cdt][cdn];
        data.custom_fob_value = data.net_amount - data.custom_freight - data.custom_insurance;
        frm.refresh_field('items');
    }
})
