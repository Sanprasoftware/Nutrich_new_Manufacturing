...
def get_data(filters):
    ...
    query = """
        SELECT
            ...
            SUM(CASE WHEN sle.voucher_type = 'Stock Entry' AND se.stock_entry_type NOT IN ('Manufacture', 'Repack') AND sle.actual_qty < 0 AND sle.posting_date BETWEEN %(from_date)s AND %(to_date)s THEN ABS(sle.actual_qty) ELSE 0 END) as others_out_qty,
